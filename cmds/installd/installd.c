...
{
    return install(arg[0], atoi(arg[1]), atoi(arg[2]), atoi(arg[3]), atoi(arg[4])); /* pkgname, uid, gid */
}
...
{
    return uninstall(arg[0], atoi(arg[1]), atoi(arg[2])); /* pkgname */
}

static int do_rename(char **arg, char reply[REPLY_MAX])
{
    return renamepkg(arg[0], arg[1], atoi(arg[2]), atoi(arg[3])); /* oldpkgname, newpkgname */
}
...
{
    return delete_cache(arg[0], atoi(arg[1]), atoi(arg[2])); /* pkgname */
}
...
{
    return delete_user_data(arg[0], atoi(arg[1]), atoi(arg[2])); /* pkgname */
}
