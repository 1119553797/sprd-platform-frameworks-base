...
bool ColorConverter::isValid() const {
    ...
    switch (mSrcFormat) {
        ...
        case OMX_SPRD_COLOR_FormatYUV420SemiPlanar:
        case OMX_SPRDMP4V_COLOR_FormatYUV420Planar:
        case OMX_SPRDH264_COLOR_FormatYVU420Planar:
            return true;
...
void ColorConverter::convert(
    ...
    switch (mSrcFormat) {
        ...
        case OMX_SPRDMP4V_COLOR_FormatYUV420Planar:
            convertSPRDYUV420Planar(width, height, 16, 16, 16, 16, srcBits, srcSkip, dstBits, dstSkip);
            break;
            
        case OMX_SPRDH264_COLOR_FormatYVU420Planar:
            convertSPRDYUV420Planar(width, height, 24, 24, 24, 24, srcBits, srcSkip, dstBits, dstSkip);
            break;
...
void ColorConverter::convertSPRDYUV420Planar(
        size_t width, size_t height,
    ...
    const uint8_t *src_u =
        (const uint8_t *)src_y + (width+left+right) * (height+top+bottom);

    const uint8_t *src_v =
        (const uint8_t *)src_u + ((width+left+right) / 2) * ((height+top+bottom) / 2);


    src_y += top * (width+left+right) + left;
    src_u += top/2 * (width+left+right)/2 +left/2;
    src_v += top/2 * (width+left+right)/2 +left/2;
    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; x += 2) {
            // B = 1.164 * (Y - 16) + 2.018 * (U - 128)
            // G = 1.164 * (Y - 16) - 0.813 * (V - 128) - 0.391 * (U - 128)
        ...
        src_y += width+left+right;

        if (y & 1) {
            src_u += (width+left+right) / 2;
            src_v += (width+left+right) / 2;
        }
