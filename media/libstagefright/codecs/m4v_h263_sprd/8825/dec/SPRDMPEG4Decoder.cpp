...
void SPRDMPEG4Decoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    if(mChangeToSwDec){
        ...
        if(!openDecoder("libstagefright_m4vh263dec_sprd.so")){
            ALOGE("onQueueFilled, open  libstagefright_m4vh263dec_sprd.so failed.");
...
//        dump_bs((uint8_t *)dec_in.pStream,  dec_in.dataLen);
        ...
        ALOGI("%s, %d, decRet: %d, frameEffective: %d, pOutFrameY: %0x", __FUNCTION__, __LINE__, decRet, dec_out.frameEffective, dec_out.pOutFrameY);
