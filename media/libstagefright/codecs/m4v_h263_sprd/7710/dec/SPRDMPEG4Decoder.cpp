...
status_t SPRDMPEG4Decoder::initDecoder() {
    ...
    mHandle->VSP_fluchCacheCb = fluchCacheWrapper;
    ...
    MMDecRet success = (*mMP4DecInit)( mHandle, &codec_buf);
...
//        dump_bs((uint8_t *)dec_in.pStream,  dec_in.dataLen);
        ...
        ALOGI("%s, %d, decRet: %d, frameEffective: %d, pY: %0x, mDecoderSwFlag: %d",
            __FUNCTION__, __LINE__, decRet, dec_out.frameEffective, dec_out.pOutFrameY, mDecoderSwFlag);
...
bool SPRDMPEG4Decoder::portSettingsChanged() {
    ...
    if(mDecoderSwFlag){
        if (mCropRight != disp_width + SW_EXTENTION_SIZE - 1
            ...
            mCropLeft = SW_EXTENTION_SIZE;
            mCropTop = SW_EXTENTION_SIZE;
    ...
    if(!mDecoderSwFlag){
        if (!((buf_width <= 720 && buf_height <= 576) || (buf_width <= 576 && buf_height <= 720)) 
            || ((buf_width <= 320 && buf_height <= 240) || (buf_width <= 240 && buf_height <= 320))){
            mDecoderSwFlag = true;
            mChangeToSwDec = true;
            ret = true;  
        }
...
int32_t SPRDMPEG4Decoder::fluchCacheWrapper(
        void* aUserData,int* vaddr,int* paddr,int size)
{
    return static_cast<SPRDMPEG4Decoder *>(aUserData)->fluchCache(vaddr, paddr, size);
}
...
int SPRDMPEG4Decoder::fluchCache (int* vaddr,int* paddr,int size)
{
...
//    ALOGI("%s, %d, Frm_width_align: %d, Frm_height_align: %d", __FUNCTION__, __LINE__, Frm_width_align, Frm_height_align);
	...
	//}
        ...
        extra_mem_size[HW_NO_CACHABLE] += 10*1024;	//rsv
        extra_mem_size[HW_CACHABLE] += 10*1024;	//rsv
    }
...
//           OMX_MP4DEC_DEBUG ("%s: ext mem pmempool %x,%x,%x,%x", __FUNCTION__, iCMDbufferPmemHeap->getHeapID(),iCMDbufferPmemHeap->base(),phy_addr,buffer_size);
    ...
    extra_mem_size[SW_CACHABLE] +=  1024; //rsv	
    if (mDecoderSwFlag)
    {
        int32 ext_size_y = (mb_num_x * 16 + 16*2) * (mb_num_y * 16 + 16*2);
        int32 ext_size_c = ext_size_y >> 2;
        ...
        //added for BUG#197476
        extra_mem_size[SW_CACHABLE] += mb_num_total * 7* sizeof(int32);
        extra_mem_size[SW_CACHABLE] += mb_num_total * sizeof(uint8);
    ...
    mCodecExtraBuffer = (uint8_t *)malloc(extra_mem_size[SW_CACHABLE]+4);
    mCodecExtraBufferMalloced = true;

    extra_mem[SW_CACHABLE].common_buffer_ptr =(uint8 *)( ((int)mCodecExtraBuffer+3)&(~0x3));
    extra_mem[SW_CACHABLE].size = extra_mem_size[SW_CACHABLE];

    if(mDecoderSwFlag  == true) 
    {
	(*mMP4DecMemCacheInit)(/* (VideoDecControls *)decCtrl*/ ((SPRDMPEG4Decoder *)this)->mHandle,&extra_mem[SW_CACHABLE]);
    }
    ...
    if(mMP4DecSetCurRecPic == NULL){
     ...
     if (mDecoderSwFlag ==  true)
     {
	    ...
	    if(mMP4DecMemCacheInit == NULL){
	        ALOGE("Can't find MP4DecMemCacheInit in %s",libName);
	        dlclose(mLibHandle);
	        mLibHandle = NULL;
	        return false;
	    }
