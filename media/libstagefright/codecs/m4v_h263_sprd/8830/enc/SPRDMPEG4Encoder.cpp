...
OMX_ERRORTYPE SPRDMPEG4Encoder::initEncParams() {
    ...
    unsigned int size_inter = MP4ENC_INTERNAL_BUFFER_SIZE;
    ...
    if ((*mMP4EncInit)(&InterMemBfr, &ExtraMemBfr,&StreamMemBfr, &mEncInfo)) {
...
void SPRDMPEG4Encoder::onQueueFilled(OMX_U32 portIndex) {
    ...
    while (!mSawInputEOS && !inQueue.empty() && !outQueue.empty()) {
        ...
        if (inHeader->nFilledLen > 0) {
            ...
            ALOGI("MP4EncStrmEncode[%lld] %dms, in {0x%p-0x%p, %dx%d}, out {0x%p-%d}", mNumInputFrames, (unsigned int)((end_encode-start_encode) / 1000000L), py, py_phy, mVideoWidth, mVideoHeight, vid_out.pOutBuf, vid_out.strmSize);
