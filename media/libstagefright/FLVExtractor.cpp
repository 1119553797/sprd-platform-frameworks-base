...
status_t FLVExtractor::FLVSource::read(
    ...
    *buffer = NULL;
    ...
    for (;;) {
        ...
        //LOGE("getTagInfo offset:%4lld, size:%d, trackID:%d,  tagId:%d,time:%4lld", offset, size,mTrackIndex, mTagIndex, timeUs);
        if (err != OK) {
...
status_t FLVExtractor::parseTagHeaders(off64_t offset, off64_t size) {
    ...
    //LOGE("parseTagHeaders hdr:0x%x%x%x%x%x%x%x%x%x", hdr[0],hdr[1],hdr[2],hdr[3],hdr[4],hdr[5],hdr[6],hdr[7],hdr[8] );

    if(hdr[4]&0x4) {//Audio tags are present
        LOGE(" detect audio tags");
        mTracks.push();
...
status_t FLVExtractor::parseTag(off_t offset, off64_t size, int depth) {
    ...
    //LOGE("parseTagHeaders hdr:0x%x%x%x%x%x%x%x%x%x", tmp[4],tmp[5],tmp[6],tmp[7],tmp[8],tmp[9],tmp[10],tmp[11],tmp[15]);
    type    = tmp[4];
    len    = (tmp[5] << 16) | (tmp[6] << 8) | (tmp[7]);
    //*p_pts  = (tmp[8] << 16) | (tmp[9] << 8) | (tmp[10]);
    flags   = tmp[4 + SIZE_OF_TAG_HEAD];

    //LOGE("parseTag type:0x%x, len:0x%x, flags:0x%x", type, len, flags);
    ...
    for( uint32_t i=0; i<mTracks.size(); i++) {
        ...
        if( track->mKind == Track::VIDEO )
            ...
            LOGE("parseTag error, track is invalid");
    }
    ...
    if( Vtrack) {
        ...
        for(;;) {            
            ...
            //LOGE("parseVideoTagHeaders hdr:0x%x,%x,%x,%x,%x,%x,%x,%x,%x", tmp[4],tmp[5],tmp[6],tmp[7],tmp[8],tmp[9],tmp[10],tmp[11],tmp[15]);
            type    = tmp[4];
            len    = (tmp[5] << 16) | (tmp[6] << 8) | (tmp[7]);
            flags   = tmp[4 + SIZE_OF_TAG_HEAD];
   ...
   if(Atrack) {
        ...
        for(;;) {            
            ...
            //LOGE("parseAudioTagHeaders hdr:0x%x,%x,%x,%x,%x,%x,%x,%x,%x", tmp[4],tmp[5],tmp[6],tmp[7],tmp[8],tmp[9],tmp[10],tmp[11],tmp[15]);
            type    = tmp[4];
            len    = (tmp[5] << 16) | (tmp[6] << 8) | (tmp[7]);
            flags   = tmp[4 + SIZE_OF_TAG_HEAD];

            if(FLV_TAG_TYPE_AUDIO==type) {
                LOGE("get audio tag Header!");
...
ssize_t FLVExtractor::flv_read_metabody(off_t offset)
{
    ...
    CHECK (amf_get_string(offset+1, buffer, sizeof(buffer)) > 11);
    CHECK (!strcmp((const char *)buffer, "onMetaData"));

    offset += (1+12);
    //parse the second object (we want a mixed array)
    //LOGE("flv_read_metabody, offset:%d", offset);
...
ssize_t FLVExtractor::amf_get_string(uint32_t offset, uint8_t *buffer, int32_t buffsize)
{
   ...
   //LOGE("amf_get_string keylen:%d", length);
    if (length >= buffsize) {
        return -1;
    }
...
ssize_t FLVExtractor::amf_parse_object(const char *key, uint32_t offset, int depth)
{
    ...
    //LOGE("amf_parse_object,offset:%d, tmp:%x, %x, %x, %x", offset, tmp[0],tmp[1],tmp[2],tmp[3]);
    amf_type = (AMFDataType)tmp[0];

    offset += 1;
    //LOGE("amf_parse_object, amf-type:0x%x", amf_type);
    switch(amf_type) {
        case AMF_DATA_TYPE_NUMBER:
            mDataSource->readAt(offset, tmp, 8);
            //LOGE("amf_parse_object,tmp:%x, %x, %x, %x,%x, %x, %x, %x", tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],tmp[6],tmp[7]);
            offset += 8;
            num_val = av_int2dbl(U64_AT(tmp));
            //LOGE("AMF_DATA_TYPE_NUMBER, %4llf", num_val);
            break;
        ...
        case AMF_DATA_TYPE_STRING:
            if((len = amf_get_string(offset, str_val, sizeof(str_val))) < 0)
                return -1;
            offset += len;
            break;
   ...
   */
        ...
        case AMF_DATA_TYPE_MIXEDARRAY:
            ...
            //LOGE("AMF_DATA_TYPE_MIXEDARRAY, array_num:%d",array_num );
    ...
    if (depth == 1 && key)
    {
        ...
        //LOGE("amf_parse_object, key:%s", key);
        ...
        if(amf_type == AMF_DATA_TYPE_BOOL) {
            ...
            {
                LOGE("amf_parse_object, duration:%.2fs", num_val);
                if(vtrack) {
            ...
            {
               LOGE("amf_parse_object, width:%4llf", num_val);
               if(vtrack )
                   vtrack->mMeta->setInt32(kKeyWidth, num_val);        
            }
            else if(!strcmp(key, "height") && num_val > 0)
            {
                LOGE("amf_parse_object, height:%4llf", num_val);
            ...
            {
                LOGE("amf_parse_object, audiosamplerate:%4llf", num_val);
...
status_t FLVExtractor::getTagInfo(
    ...
    if (trackIndex >= mTracks.size()) {
        LOGE("trackId:%d, size:%d", trackIndex, mTracks.size());
        return -ERANGE;
    }
    uint32_t tagType;
    Track *track = &mTracks.editItemAt(trackIndex);
    //LOGE("getTagInfo trackID:%d, tagId:%d, tagNum:%d",trackIndex, tagIndex, track->mTags.size());
    while(tagIndex >= track->mTags.size()) {
            uint8_t tmp[4+SIZE_OF_TAG_HEAD+1];
            ssize_t n = mDataSource->readAt(track->mCurTagPos, tmp, 4+SIZE_OF_TAG_HEAD+1);
            //LOGE("getTagInfo pos:%x, tmp:%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x", track->mCurTagPos,
            ...
            //LOGE("getTagInfo push pos:%x, info:%x ", track->mCurTagPos, info->mOffset);
    ...
    *size = 0;
    ...
    //LOGE("getTagInfo pos:%x, tmp:%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x", info.mOffset,
    //    tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],tmp[6],tmp[7],tmp[8],tmp[9],tmp[10],tmp[11],tmp[12],tmp[13],tmp[14],tmp[15]);
    ...
    *tagTimeUs = ( (tmp[8] << 16) | (tmp[9] << 8) | (tmp[10]) )*1000;
    ...
    //LOGE("getTagInfo timeUs:%4lld", *tagTimeUs);
