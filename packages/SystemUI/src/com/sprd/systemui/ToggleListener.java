...
// fix bug 195755 to open Settings when toggle button was clicked on 20130730 end
   ...
   @Override
    ...
    /*private void updateDataNetworkButton() {
        ...
        if (defaultPhoneId > -1 && defaultPhoneId < mTelephonyManagers.length) {
            simIsReady = mTelephonyManagers[defaultPhoneId].getSimState() == TelephonyManager.SIM_STATE_READY;
        }
        ...
        if (numPhones > 1) {
            mDataDefaultNetworkOn = mConnManager.getMobileDataEnabledByPhoneId(defaultPhoneId);
        } else {
    ...
    /*private void toggleAutoDataConnection() {
        ...
        b.setSingleChoiceItems(simsAdapter, -1, null);
