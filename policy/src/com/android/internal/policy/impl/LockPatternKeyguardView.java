...
//                && (mUpdateMonitor.getSimState() == IccCard.State.ABSENT);
    ...
    public void wakeWhenReadyTq(int keyCode) {
        ...
        if (keyCode == KeyEvent.KEYCODE_MENU && isSecure() && (mMode == Mode.LockScreen)
                /* PUK Input Add && (mUpdateMonitor.getSimState() != IccCard.State.PUK_REQUIRED)*/) {
...
//        final IccCard.State simState = mUpdateMonitor.getSimState();
    ...
    private void showTimeoutDialog() {
        ...
        String message = mContext.getString(
                ...
                mUpdateMonitor.getFailedAttempts(),
                timeoutInSeconds);
        //add by niezhong for NEWMS00118697 09-17-11 begin
        if((""+(UnlockMode.Password)).equals(""+getUnlockMode())) {
        	message = mContext.getString(
                    R.string.lockscreen_too_many_password_failed_attempts_dialog_message,
                    mUpdateMonitor.getFailedAttempts(),
                    timeoutInSeconds);
        }
        //add by niezhong for NEWMS00118697 09-17-11 end
