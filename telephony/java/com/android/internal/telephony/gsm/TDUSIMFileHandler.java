...
public final class TDUSIMFileHandler extends SIMFileHandler implements
    ...
    /*
     * static class LoadLinearFixedContext { int efid; int recordNum,
     * recordSize, countRecords; boolean loadAll; Message onLoaded;
     * ArrayList<byte[]> results; LoadLinearFixedContext(int efid, int
     * recordNum, Message onLoaded) { this.efid = efid; this.recordNum =
     * recordNum; this.onLoaded = onLoaded; this.loadAll = false; }
    ...
    public TDUSIMFileHandler(IccCard card, String aid, PhoneBase mPhone) {
        super(card, aid, mPhone);
        initDualMapFileSet();
    }
     ...
     * 
     * @param fileid EF id
     * @param onLoaded ((AsyncResult)(onLoaded.obj)).result is the byte[]
     */
    ...
    protected String getEFPathofUsim(int efid) {
        ...
        Log.i(LOG_TAG, "getEFPathofUsim false , try again pathFirst " + pathFirst + ", pathSecond "
                + pathSecond + ", pathLast " + pathLast);
        ...
        try {
            switch (msg.what) {
                ...
                case EVENT_GET_EF_LINEAR_RECORD_SIZE_DONE:
                    ...
                    if (isUsim)
                    {
                        ...
                        if (iccException != null || (fileId == EF_PBR && !isDataValid(data))) {
                            // Log.i(LOG_TAG,
                            // "EVENT_GET_EF_LINEAR_RECORD_SIZE_DONE pathNum "+
                            // pathNum);
                        ...
                        // Log.i(LOG_TAG,"EVENT_GET_EF_LINEAR_RECORD_SIZE_DONE index "+
                        // index);
                        if (index < 2) {
                        ...
                        for (int i = 0; i < 3; i++) {
                            Log.i(LOG_TAG,
                                    "EVENT_GET_EF_LINEAR_RECORD_SIZE_DONE  recordSize[" + i + "] "
                                            + recordSize[i]);
                        }
                ...
                case EVENT_GET_RECORD_SIZE_DONE:
                    ...
                    logd("EVENT_GET_RECORD_SIZE_DONE, path " + path + ",  lc.recordNum:"
                            + lc.recordNum);
                ...
                case EVENT_READ_RECORD_DONE:
                    ...
                    if (!lc.loadAll) {
                        ...
                        if (lc.recordNum > lc.countRecords) {
                            ...
                            path = getEFPath(lc.efid);
                            // logd("EVENT_READ_RECORD_DONE, fileid:"+lc.efid+"("+Integer.toHexString(lc.efid)+")");
                            // logd("EVENT_READ_RECORD_DONE, path:"+path+",  lc.recordNum:"+
                            // lc.recordNum);
                            phone.mCM.iccIO(COMMAND_READ_RECORD, lc.efid,
                                    path, lc.recordNum,
