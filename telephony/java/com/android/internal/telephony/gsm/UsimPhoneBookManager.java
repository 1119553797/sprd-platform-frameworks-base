...
public class UsimPhoneBookManager extends IccThreadHandler implements IccConstants {
    ...
    public synchronized ArrayList<AdnRecord> loadEfFilesFromUsim() {
        synchronized (mLock) {
            ...
            // from the USIM.
            Log.i(LOG_TAG, "loadEfFilesFromUsim, mPbrFile:" + mPbrFile);
            if (mPbrFile == null) {
                readPbrFileAndWait();
            }
            ...
            mDoneAdnCount = 0;
            Log.i(LOG_TAG, "loadEfFilesFromUsim, numRecs:" + numRecs);

            Log.i(LOG_TAG, "loadEfFilesFromUsim, mTotalSize:" + mTotalSize);
            if (mTotalSize == null) {
            ...
            for (int i = 0; i < numRecs; i++) {
                Log.i(LOG_TAG, "loadEfFilesFromUsim, the current record num is:" + i);
    ...
    public int getEfIdByTag(int recordNum, int fileTag) {
        ...
        if (mPbrFile == null) {
            Log.e(LOG_TAG, "getEfIdByTag error, Pbr file is empty");
            return -1;
        }
        fileIds = mPbrFile.mFileIds.get(recordNum);

        if (fileIds == null) {
            Log.e(LOG_TAG, "getEfIdByTag error, fileIds is empty");
            return -1;
        }
    ...
    public int findEFGasInfo() {
        ...
        if (fileIds == null) {
            Log.i(LOG_TAG, "findEFGasInfo fileIds == null");
            return -1;
        }
    ...
    public synchronized int[] getAdnRecordsSize() {
        ...
        synchronized (mLock) {
            ...
            // Log.i(LOG_TAG, "getEFLinearRecordSize size" + size[2]);
            if (mTotalSize != null) {
    ...
    private int[] readAdnFileSizeAndWait(int recNum) {
        ...
        Log.i(LOG_TAG, "readAdnFileSizeAndWait");
        synchronized (mLock) {
            if (mPbrFile == null) {
                readPbrFileAndWait();
            }
        ...
        Log.i(LOG_TAG, "readAdnFileSizeAndWait, the current recNum is " + recNum);
    ...
    private void readPbrFileAndWait() {
        Log.i(LOG_TAG, "readPbrFileAndWait");
    ...
    private int[] readAdnFileAndWait(int recNum) {
        Log.i(LOG_TAG, "readAdnFileAndWait");
        synchronized (mLock) {
            if (mPbrFile == null) {
                readPbrFileAndWait();
            }
    ...
    private void readGrpFileAndWait(int recNum) {
        Log.i(LOG_TAG, "readGrpFileAndWait");
        synchronized (mLock) {
            if (mPbrFile == null) {
                readPbrFileAndWait();
            }
        ...
        if (efId == null)
            return;
        mPhone.getIccFileHandler().loadEFLinearFixedAll(efId, obtainMessage(EVENT_GRP_LOAD_DONE));

        try {
            mLock.wait();
        } catch (InterruptedException e) {
            Log.e(LOG_TAG, "Interrupted Exception in readGrpFileAndWait");
        }

    }

    private void readGasFileAndWait(int recNum) {
        Log.i(LOG_TAG, "readGasFileAndWait");
        synchronized (mLock) {
            if (mPbrFile == null) {
                readPbrFileAndWait();
            }
        ...
        if (efId == null)
            return;
        mPhone.getIccFileHandler().loadEFLinearFixedAll(efId, obtainMessage(EVENT_GAS_LOAD_DONE));

        synchronized (mLock) {
            try {
                mLock.wait();
            } catch (InterruptedException e) {
                Log.e(LOG_TAG, "Interrupted Exception in readGasFileAndWait");
            }
        }
    }
    ...
    private void readIapFileAndWait(int efid) {
        Log.i(LOG_TAG, "readIapFileAndWait");
        mPhone.getIccFileHandler().loadEFLinearFixedAll(efid,
    ...
    public ArrayList<String> loadGasFromUsim() {
        ...
        if (mGasFileRecord == null) {
            Log.e(LOG_TAG, "Error: mGasFileRecord file is empty");
            return null;
        }
    ...
    private void updatePbcAndCc() {
        Log.i(LOG_TAG, "update EFpbc begin");
        Map<Integer, Integer> fileIds;
        fileIds = mPbrFile.mFileIds.get(0);
        if (fileIds == null || fileIds.isEmpty())
            return;
        ...
        if (efPbcId == null)
            return;
        Log.i(LOG_TAG, " USIM_EFPBC_TAG = " + Integer.toHexString(efPbcId));
        int changeCounter = 0;
        mFh.loadEFLinearFixedAll(efPbcId, obtainMessage(EVENT_LOAD_EF_PBC_RECORD_DONE));
        try {
            mLock.wait();
        } catch (InterruptedException e) {
            Log.e(LOG_TAG, "Interrupted Exception in updatePbcAndCc");
        }
        if (mPbcFileRecord == null) return;
        ...
        Log.i(LOG_TAG, "update EFpbc end, changeCounter " + changeCounter);
    ...
    private void updateAdnRecord(int num) {
        ...
        if (mIapFileRecord != null) {
            ...
            Log.i(LOG_TAG, "updateAdnRecord mIapRecordSizeArray[num] : "
                    + mIapFileRecord.size());
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_GRP_LOAD_DONE:
                //log("Loading USIM Grp records done, mGrpFileRecord:"+mGrpFileRecord);
                ar = (AsyncResult) msg.obj;
                if (ar.exception == null) {
                    if(mGrpFileRecord == null){
            ...
            case EVENT_EF_CC_LOAD_DONE:
                ...
                Log.i(LOG_TAG,"EVENT_EF_CC_LOAD_DONE data "+ IccUtils.bytesToHexString(data));
                ...
                Log.i(LOG_TAG,"EVENT_EF_CC_LOAD_DONE counter "+ IccUtils.bytesToHexString(counter));
                ...
                switch (tag) {
                    ...
                    case USIM_EFSNE_TAG:
                        ...
                        if (tag == USIM_EFEMAIL_TAG) {
                            ...
                            Log.i(LOG_TAG, "parseEf   email  efid " +Integer.toHexString(efid) +"  TAG  " + Integer.toHexString(parentTag)
                        ...
                        if (tag == USIM_EFANR_TAG) {
                            Log.i(LOG_TAG, "parseEf   ANR  efid " +Integer.toHexString(efid) +"  TAG  " + Integer.toHexString(parentTag)
                                    + "  tagNumberWithinParentTag " +Integer.toHexString(tagNumberWithinParentTag));
