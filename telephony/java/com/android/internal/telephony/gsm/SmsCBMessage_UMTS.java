...
public class SmsCBMessage_UMTS {
    private static final boolean LOCAL_DEBUG = true;
    private static final String TAG = "SmsCBMessage_UMTS";
    ...
    private static SmsCBPage_UMTS mPage;
    ...
    public SmsCBMessage_UMTS(Context context, ContentResolver contentResolver) {
        mContext = context;
        mContentResolver = contentResolver;
    }

    public static class SmsCBPage_UMTS {
    ...
    /*
     * private static SmsCBPage_UMTS getOneSmsCBPage(String msg, int length ) {
     * Log.i(TAG, "getOneSmsCBPage length" + msg.length()); String
     * midDecodeData; byte[] pages = msg.getBytes(); ParseValue retSn =
     * getValue(pages); String mid = msg.substring(retSn.index); byte[] byteMid
     * = mid.getBytes(); ParseValue retMid = getValue(byteMid); String midDcs =
     * mid.substring(retMid.index); byte[] byteDcs = midDcs.getBytes();
     * ParseValue retDcs = getValue(byteDcs); String midPage =
     * midDcs.substring(retDcs.index); byte[] bytePage = midPage.getBytes();
     * ParseValue retPage = getValue(bytePage); String midPages =
     * midPage.substring(retPage.index); byte[] bytePages = midPages.getBytes();
     * ParseValue retPages = getValue(bytePages); int contentPos = 0; for (int i
     * = 0; i < pages.length - 1; i++) { if (pages[i] == CR && pages[i + 1] ==
     * LF) { contentPos = i + 2; break; } } Log.i(TAG, "getSmsCBPage pos" +
     * contentPos); Log.i(TAG, "getSmsCBPage pos" + contentPos); String
     * DecodeData = msg.substring(contentPos); byte[] byteDecodeData =
     * DecodeData.getBytes(); int encodingType = ENCODING_UNKNOWN; mPage = new
     * SmsCBPage_UMTS(); byte[] byteSn = new byte[2]; byteSn[1] =
     * (byte)(retSn.ret &0xff); byteSn[0] = (byte)(retSn.ret>>0x8); mPage.gs =
     * byteSn[0] >> 6 & 0x3; mPage.messageCode = ((byteSn[0] & 0x3F) <<4)|
     * ((byteSn[1] >> 4) & 0xF); mPage.updateNum = byteSn[1] & 0xF; byte[]
     * byteMsgId = new byte[2]; byteMsgId[0] = (byte)(retMid.ret&0xff);
     * byteMsgId[1] = (byte)(retMid.ret>>0x08); mPage.msgId = byteMsgId[0] |
     * (byteMsgId[1] << 8); mPage.dcs = retDcs.ret; mPage.sequenceNum =
     * retPage.ret; mPage.totalNum = retPages.ret; mPage.decodedData =
     * byteDecodeData; Log.i(TAG, "getOneSmsCBPage gs :" + mPage.gs +
     * "messageCode : "+mPage.messageCode + "updateNum: "+mPage.updateNum+ //
     * "msgId: "
     * +mPage.msgId+"dcs : "+mPage.dcs+"sequenceNum: "+mPage.sequenceNum
     * +"totalNum: "+mPage.totalNum); mDcs = decodeSmsCBCds(mPage.dcs,
     * mPage.decodedData, false); mPage.langId = mDcs.languageId; midDecodeData
     * = msg.substring(contentPos); if(mDcs.byteOffset > 0 ){ int newContentPos
     * = 0; int i =0; for (i = contentPos; i < contentPos+3 ; i++) { if
     * (pages[i] == CR ) { newContentPos = i+1; break; } } if(i< contentPos+3){
     * midDecodeData = msg.substring(newContentPos); } } Log.v(TAG,
     * "getOneSmsCBPage  midDecodeData: " +midDecodeData ); //byte[]
     * byteDecodeData = IccUtils.hexStringToBytes(midDecodeData); //Log.v(TAG,
     * "getOneSmsCBPage ENCODING_16BIT byteDecodeData.length: "
     * +byteDecodeData.length ); //mPage.decodedData = byteDecodeData;
     * Log.i(TAG, "getOneSmsCBPage encodingType" + encodingType); switch
     * (mDcs.alphabetType) { case ENCODING_UNKNOWN: case ENCODING_8BIT:
     * mPage.content = null; break; case ENCODING_7BIT: mPage.content =
     * midDecodeData;//new String(mPage.decodedData); break; case
     * ENCODING_16BIT: byte atContentData[] =
     * IccUtils.hexStringToBytes(midDecodeData); for(int i=0;
     * i<atContentData.length; i++){ Log.v(TAG,
     * "getOneSmsCBPage ENCODING_16BIT atContentData: " +atContentData[i] ); }
     * mPage.content = getUserDataUCS2(atContentData, 0);// (mDcs.byteOffset)
     * break; } Log.v(TAG, "getOneSmsCBPage SMS message body (raw): " +
     * mPage.content); return mPage; }
     */
    /*
     * get page information in pdu mode ,see TS 23.041 9.4.1.2
     */
    public static byte[][] getSmsCBPage(String msg, int length, Context context,
            ContentResolver contentResolver, int phoneID) {
    ...
    // String pdu =
    // "0100024010010F537A584E07BDCD2068F85C8E81A86F3808590E93D96977790E7A9B41747419440EE76520EA1B0E4296C36476DA5D9E83DE66101D5D0691C3791988FA868390E530999D7697E7A0A29B0C7A9A41D0F0B91C01";
    // byte[][] bytePages = getSmsCBPage(pdu,pdu.length());
    // }
    /*
     * get page information in pdu mode ,see TS 23.041 9.4.2
     */
    // public static byte[][] getSmsCBPage(String msg, int length/*,Context
    ...
    // String atString = new String(contentPages);
    // Log.i(TAG, "getSmsCBPage atString" + atString);
    // byte atContentData[] = hexStringToBytes(atString);
    // mDcs = decodeSmsCBCds(atContentData[5], atContentData, true);
    //
    // Log.i(TAG, "getSmsCBPage mDcs" + mDcs.toString());
    //
    //
    ...
    private static byte[][] convertToBytes(SmsCBPage_UMTS[] pages, int totalNum) {
    ...
    public static byte[][] processSmsCBPage(SmsCBPage_UMTS page, int phoneID) {
        Uri RawUri = Uri.parse("content://sms/cbsmsraw");

        String[] CB_RAW_PROJECTION = new String[] {
                "gs", "message_code",
                "update_num", "message_id", "dcs", "count", "sequence", "langId",
                "content"
        };

        SmsCBPage_UMTS[] pages;
        ...
        try {
            if (page.totalNum > 1) {
                ...
                if (cursor != null) {
                    ...
                    Log.i(TAG, "processSmsCBPage cursorCount " + cursorCount + "page.totalNum "
                            + page.totalNum);
                    pages = new SmsCBPage_UMTS[cursorCount + 1];

                    for (i = 0; i < cursorCount; i++) {
                        cursor.moveToNext();
                        pages[i] = new SmsCBPage_UMTS();
                ...
                pages = new SmsCBPage_UMTS[1];
                pages[0] = new SmsCBPage_UMTS();
                pages[0].gs = page.gs;
    ...
    private static SmsCBDcs decodeSmsCBCds(int dcsData, byte[] msg,
                    ...
                    case 0x10:
                        ...
                        // 0001 0000
                            case 0x00:
                                ...
                                // two-character
                                // representation
                                // of the language encoded according to ISO 639
                                // [12],
                                // followed by a CR character.
                                // The CR character is then followed by 90
                                // characters of
                                // text.
                                ...
                                mDcs.languageIdPresent = true;
                                if (isPdu) {
                                    lang_id_h = (byte) (msg[7] & 0x7f);
                                    lang_id_l = (byte) ((msg[8] & 0x3f) << 1)
                                            | ((msg[7] & 0x80) >> 7);
                                } else {
                            ...
                            // 0001 0001
                            case 0x01:
                                // The message starts with a two GSM 7-bit
                                // default alphabet
                                // character representation
                                // of the language encoded according to ISO 639
                                // [12]. This
                                // is padded to the octet
                                // boundary with two bits set to 0 and then
                                // followed by 40
                                // characters of UCS2-encoded message.
                                mDcs.byteOffset = 9;
                                mDcs.languageIdPresent = true;
                                if (isPdu) {
                                    lang_id_h = (msg[7] & 0x7f);
                                    lang_id_l = (byte) ((msg[8] & 0x3f) << 1)
                                            | ((msg[8] & 0x80) >> 7);
                                } else {
        ...
        if (!isPdu) {
            ...
            mDcs.byteOffset = mDcs.byteOffset - 7;
     ...
     *            User Data.
     */
    private static SmsCBPage_UMTS parseUserData(byte[] msg, int length, SmsCBDcs dcs) {
        int encodingType = ENCODING_UNKNOWN;
        mPage = new SmsCBPage_UMTS();
        mPage.messageType = msg[0];
        // mPage.msgId = msg[1] << 8 | msg[2];
        // mPage.serialNumber = msg[3] << 8 | msg[4];
        // mPage.dcs = msg[5];

        mPage.gs = msg[3] >> 6 & 0x3;
        mPage.messageCode = ((msg[3] & 0x3F) << 4) | ((msg[4] >> 4) & 0xF);
        mPage.updateNum = msg[4] & 0xF;
        mPage.msgId = msg[1] << 8 | msg[2];
        mPage.dcs = msg[5];
        // mPage.sequenceNum = (msg[5] >> 4) & 0x0F;
        mPage.totalNum = msg[6];// & 0x0F;
        ...
        mPage.sequenceNum = msg[length - 1];
        mPage.content = "";
        encodingType = dcs.alphabetType;

        Log.i(TAG, "parseUserData gs:" + mPage.gs + " messageCode:" + mPage.messageCode
                + " updateNum:" + mPage.updateNum +
                " msgId:" + mPage.msgId + " dcs: " + mPage.dcs + " sequenceNum:"
                + mPage.sequenceNum + " totalNum:" + mPage.totalNum);
        switch (encodingType) {
            case ENCODING_UNKNOWN:
            case ENCODING_8BIT:
                break;
            case ENCODING_7BIT:
                int len = ((length - dcs.byteOffset) * 8) / 7;
                mPage.content = GsmAlphabet.gsm7BitPackedToString(msg,
                        dcs.byteOffset, len, dcs.bitOffset, 0, 0);
                break;
            case ENCODING_16BIT:
