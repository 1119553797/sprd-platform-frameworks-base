...
public class VideoConnection extends Connection {
    ...
    int index;          // index in VideoCallTracker.connections[], -1 if unassigned
    ...
    /*package*/
    VideoConnection (Context context, DriverCall dc, VideoCallTracker ct, int index) {
    ...
    /*package*/
    VideoConnection (Context context, String dialString, VideoCallTracker ct, VideoCall parent) {
        createWakeLock(context);
        acquireWakeLock();
        ...
        this.parent = parent;
        parent.attachFake(this, VideoCall.State.DIALING);
    }
    ...
    public VideoCall.State getState() {
        if (disconnected) {
            return VideoCall.State.DISCONNECTED;
        } else {
    ...
    public void proceedAfterWaitChar() {
        if (postDialState != PostDialState.WAIT) {
            Log.w(LOG_TAG, "VideoConnection.proceedAfterWaitChar(): Expected "
    ...
    update (DriverCall dc) {
        ...
        boolean wasHolding = (getState() == VideoCall.State.HOLDING);
