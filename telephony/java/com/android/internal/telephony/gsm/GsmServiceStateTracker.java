...
final class GsmServiceStateTracker extends ServiceStateTracker {
    ...
    protected void setPowerStateToDesired(boolean force) {
        Log.d(LOG_TAG, "setPowerStateToDesired " + "phone="+phone.getPhoneId()+" mDesiredPowerState="+mDesiredPowerState+" cm.getRadioState().isOn="+cm.getRadioState().isOn()+" force="+force);
        // If we want it on and it's off, turn it on
        if (mDesiredPowerState
            && (cm.getRadioState() == CommandsInterface.RadioState.RADIO_OFF)) {
            if(force ||
               phone.getIccCard().getIccCardState() != IccCard.State.ABSENT &&
    ...
    protected void powerOffRadioSafely() {
        // hang up all active voice calls first, modem can deactivate data call
        // when no active voice call.
        //if (phone.isInCall()) {
   ...
   public String setRadioType(int type,boolean airplane){
    ...
    private void pollStateDone() {
        if (DBG) {
            Log.d(LOG_TAG, "[" + phone.getPhoneId() + "]Poll ServiceState done: " +
    ...
    private void log(String s) {
        Log.d(LOG_TAG, "[GsmServiceStateTracker-phoneId" + phone.getPhoneId() + "] " + s);
    }
}
